...
void main(){
    ...
    vec3 planeColor = vec3(0.02, 0.02, 0.02);


	//float ambienceIntensity = 0.005;
	//vec3 ambience = lightColor * ambienceIntensity;
    vec3 ambience = vec3(0.02, 0.0, 0.05);

    vec3 diffuse = (planeColor * diffEval) * lightColor;
    vec3 specularTerm = specular * lightColor;

    vec3 directLight = intensity * (diffuse + specularTerm);
    vec3 finalColor = ambience + (shadowTerm * directLight);
